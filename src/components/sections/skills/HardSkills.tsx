...
export const HardSkills = () => {
  ...
  const categorizedSkills = useMemo(() => {
    ...
    portfolioData.hardSkills.forEach(skill => {
      ...
      if (['cybersecurity', 'security'].includes(cat)) {
        groups['cybersecurity'].push(skill);
      } else if (['ai', 'ml'].includes(cat)) {
      ...
      <div className="w-full max-w-[1400px] px-4 md:px-8 mx-auto relative z-10 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 w-full relative z-10 items-stretch">
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.cybersecurity ? "true" : undefined}
              onWheel={expanded.cybersecurity ? (e) => e.stopPropagation() : undefined}
              className={cn(
                ...
                expanded.cybersecurity ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills['cybersecurity'].slice(0, expanded.cybersecurity ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills['cybersecurity'].length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                  onClick={() => toggleExpand('cybersecurity')}
                ...
                >
                  {expanded.cybersecurity ? (
                    ...
                    <>View More ({categorizedSkills['cybersecurity'].length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
